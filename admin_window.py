from PyQt6.QtWidgets import QWidget, QVBoxLayout, QLabel, QPushButton, QHBoxLayout, QTableWidget, QTableWidgetItem, QLineEdit, QFormLayout, QMessageBox, QDateEdit, QSpinBox, QTextEdit, QComboBox, QFileDialog, QTabWidget, QTimeEdit, QGroupBox, QHeaderView
...
class AdminWindow(QWidget):
    def __init__(self, user):
        ...
        layout.addWidget(QLabel(f"Панель администратора — {user['fio']}"))

        tabs = QTabWidget()

        clients_tab = QWidget()
        clients_layout = QVBoxLayout()
        top_group = QGroupBox("Действия")
        ...
        top.addStretch()
        clients_layout.addWidget(top_group)

        self.clients_table = QTableWidget(0,6)
        self.clients_table.setHorizontalHeaderLabels(['ID','ФИО','Телефон','Email','Логин','Тип'])
        self._configure_table(self.clients_table)
        clients_layout.addWidget(self.clients_table)

        print_group = QGroupBox("Печать документов")
        print_buttons = QHBoxLayout(print_group)
        ...
        schedule_layout.addWidget(QLabel("Расписание тренировок"))

        form_group = QGroupBox("Новое занятие")
        ...
        form.addRow("Дата:", self.class_date)
        ...
        schedule_layout.addWidget(form_group)
