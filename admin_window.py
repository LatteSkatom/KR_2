...
class AdminWindow(QWidget):
    ...
    def show_membership_dialog(self):
        ...
        duration = QComboBox()
        duration.addItem("1 месяц", 1)
        duration.addItem("3 месяца", 3)
        duration.addItem("1 год", 12)
        f.addRow("Срок действия:", duration)
        ...
        def do_create():
            ...
            months = duration.currentData()
            end_date = start.date().addMonths(months)
            create_membership(cid, typ.text() or 'Месячный', start.date().toString("yyyy-MM-dd"), end_date.toString("yyyy-MM-dd"), visits.value(), 0, 'Все зоны', 'Активен', float(cost.text() or 0), self.admin_id)
